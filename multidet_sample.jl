...
total_time_s = lcm_samples ÷ fsamp_hz
...
tatm_k = 15
ttel_k = 3
tnoise_k = 35 #diversa per ogni pol -> vedi database
β_hz = 7e9
...
if rank == 0
    ...
    @assert sum(samples_per_process) == total_samples "Sum of samples per process != total samples"
end
...
# Debug
if rank == 0
    println(" get_chunk_properties results:")
    println(" detector_number: ", detector_number)
    println(" first_time: ", first_time)
    println(" last_time: ", last_time)
    println(" num_of_baselines: ", num_of_baselines)
    println(" num_of_samples: ", num_of_samples)
    println(" typeof(num_of_baselines): ", typeof(num_of_baselines))
    println(" typeof(num_of_samples): ", typeof(num_of_samples))
...
@printf("[rank %d] length(noise_tod) = %d, sum(ns) = %d\n",
        rank, length(noise_tod), sum(num_of_samples))
...
rms_list = [
      ...
      fill(sigma_scalar, num_of_samples[i])
      for i in 1:length(num_of_samples)
]

# DEBUG: Prima di chiamare build_noise_properties
...
""", num_of_polarimeters, medium, med, dev))
    ...
    if rank == 0
      let fname = out_map_root * "_cond_multi_samp.fits"
...
if rank == 0
    ...
    for (i, suffix) in enumerate(("_Q_white_rms_samp.fits", "_U_white_rms_samp.fits"))
...
if rank == 0
    out_map_name =  out_map_root*"_Q_multi_samp.fits"
    ...
    out_map_name =  out_map_root*"_U_multi_samp.fits"
    printmsg("Saving the map in \"$(out_map_name)\"\n")
    mapfile = Healpix.HealpixMap{Float64, Healpix.RingOrder}(NSIDE)
