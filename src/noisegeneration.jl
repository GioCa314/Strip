...
function generate_noisechunks_to_send(
    ...
    for cur_rank = 0:(commsize-1) 
        pol_cur_rank = cur_rank + 1 
        chunk_cur_rank = []
        ...
        for i in eachindex(chunks) 
            ...
            for j = 1:length(chunks[i])
                if chunks[i][j].pol_number == pol_cur_rank 
                   
                   first_idx = chunks[i][j].first_idx
                   last_idx = chunks[i][j].last_idx
...
function generate_noise_mpi(
    ...
    if (commsize >= total_number_of_polarimeters)
        ...
        if rank == 0
            @show noisechunks_to_send[1]
            for c in noisechunks_to_send[1]
                @printf("chunk[%d:%d] dest %d ", c.first_idx, c.last_idx, c.dest_rank)
            end
        end    
        ...
        noise = Array{Float64}(
            undef,
            round(Int, samples_per_process[rank+1]),
	    
        )
        if 0 <= rank < total_number_of_polarimeters
        ...
        for i in eachindex(noisechunks_to_send)  
            cur_rank = i - 1

            for j in eachindex(noisechunks_to_send[i])
        ...
        if rank == 0
            ...
            for i in eachindex(chunks)
                ...
                for j in eachindex(chunks[i])
                    ...
                    cur_num_noise_samples = chunks[i][j].num_of_elements
                    ...
                    if cur_detector != previous_detector  
            ...
            for j = 1:length(chunks[rank+1])
                cur_detector = chunks[rank+1][j].pol_number
                n = chunks[rank+1][j].num_of_elements
                cur_noise = Array{Float64}(undef, n)
